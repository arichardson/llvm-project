...
private:
  ...
  static constexpr uint32_t IMask = 0x200;
  static constexpr uint32_t IShift = 9;
  static constexpr uint32_t OMask = 0x400;
  static constexpr uint32_t OShift = 10;
  static constexpr uint64_t AddressSpaceMask =
      ~(CVRMask | UMask | GCAttrMask | LifetimeMask | IMask | OMask);
...
class ConstantArrayType : public ArrayType {
  ...
  static ConstantArrayType *Create(const ASTContext &Ctx, QualType ET,
                                   ...
                                   unsigned Qual, std::optional<PointerInterpretationKind> PIK);
