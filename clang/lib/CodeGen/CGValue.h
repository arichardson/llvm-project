...
public:
  ...
  static RValue get(llvm::Value *V, unsigned Alignment = 0) {
    ...
    ER.V1.setPointer(V);
    ER.V1.setInt(Scalar);
    ER.V2.setPointer(reinterpret_cast<llvm::Value *>(Alignment << AggAlignShift));
    ER.V2.setInt(false);
    return ER;
  }

  #if 0
  static RValue get(Address Addr, CodeGenFunction &CGF, unsigned Alignmnet = 0) {
    return RValue::get(Addr.emitRawPointer(CGF), Alignmnet);
  }
  #endif
