...
static void ParseConstraint(const std::string &CStr, CGIOperandList &Ops,
  ...
  if (Tok == "@traps_if_untagged") {
    std::string Name = CStr.substr(wpos + 1);
    std::pair<unsigned, unsigned> Op =
        ParseBooleanConstraintOp("@traps_if_untagged", Name);
    // TODO: should check that this is an input operand
    Ops[Op.first].TrapsIfUntaggedCapability = true;
    return;
  }
...
CodeGenInstruction::CodeGenInstruction(Record *R)
  ...
  bool HasTrapOnUntaggedOpConstraint = false;
  for (auto &Op : Operands) {
    if (Op.TrapsIfUntaggedCapability)
      HasTrapOnUntaggedOpConstraint = true;
  ...
  mayTrapOnUntaggedInput =
      R->getValueAsBitOrUnset("mayTrapOnUntaggedInput", Unset);
  if (Unset)
    mayTrapOnUntaggedInput = HasTrapOnUntaggedOpConstraint;
  defIsAlwaysTagged = R->getValueAsBit("defIsAlwaysTagged");
  defMayBeSealed = R->getValueAsBit("defMayBeSealed");
