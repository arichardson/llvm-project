...
; RUN: opt < %s -instcombine -S | llc -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+xcheri,+cap-mode,+f - -o - | FileCheck %s --check-prefix=CHECK-ASM
...
; CHECK-ASM-NEXT:    # Label of block must be emitted
; CHECK-ASM-NEXT:    auipcc ca3, %captab_pcrel_hi(memcpy)
; CHECK-ASM-NEXT:    clc ca3, %pcrel_lo(.LBB0_2)(ca3)
; CHECK-ASM-NEXT:    addi a2, zero, 17
; CHECK-ASM-NEXT:    cjalr ca3
...
; CHECK-ASM-NEXT:    # Label of block must be emitted
; CHECK-ASM-NEXT:    auipcc ca3, %captab_pcrel_hi(memcpy)
; CHECK-ASM-NEXT:    clc ca3, %pcrel_lo(.LBB1_2)(ca3)
; CHECK-ASM-NEXT:    addi a2, zero, 17
; CHECK-ASM-NEXT:    cjalr ca3
; CHECK-ASM-NEXT:    clc cra, 8(csp) # 8-byte Folded Reload
; CHECK-ASM-NEXT:    cincoffset csp, csp, 16
...
; CHECK-ASM-NEXT:    # Label of block must be emitted
; CHECK-ASM-NEXT:    auipcc ca1, %captab_pcrel_hi(strlen)
; CHECK-ASM-NEXT:    clc ca1, %pcrel_lo(.LBB2_1)(ca1)
; CHECK-ASM-NEXT:    cjalr ca1
; CHECK-ASM-NEXT:    cincoffset ca0, cs0, a0
; CHECK-ASM-NEXT:  .LBB2_2: # %entry
; CHECK-ASM-NEXT:    # Label of block must be emitted
; CHECK-ASM-NEXT:    auipcc ca1, %captab_pcrel_hi(.Lstr)
; CHECK-ASM-NEXT:    clc ca1, %pcrel_lo(.LBB2_2)(ca1)
; CHECK-ASM-NEXT:  .LBB2_3: # %entry
; CHECK-ASM-NEXT:    # Label of block must be emitted
; CHECK-ASM-NEXT:    auipcc ca3, %captab_pcrel_hi(memcpy)
; CHECK-ASM-NEXT:    clc ca3, %pcrel_lo(.LBB2_3)(ca3)
; CHECK-ASM-NEXT:    addi a2, zero, 17
; CHECK-ASM-NEXT:    cjalr ca3
...
; CHECK-IR-NEXT:    call void @llvm.memcpy.p200i8.p200i8.i32(i8 addrspace(200)* noundef nonnull align 1 dereferenceable(17) [[ENDPTR]], i8 addrspace(200)* noundef nonnull align 4 dereferenceable(17) getelementptr inbounds ([17 x i8], [17 x i8] addrspace(200)* @str, i32 0, i32 0), i32 17, i1 false) #[[ATTR3:[0-9]+]]
; CHECK-IR-NEXT:    ret void
;
entry:
...
; CHECK-ASM-NEXT:    # Label of block must be emitted
; CHECK-ASM-NEXT:    auipcc ca4, %captab_pcrel_hi(strncpy)
; CHECK-ASM-NEXT:    clc ca4, %pcrel_lo(.LBB3_2)(ca4)
; CHECK-ASM-NEXT:    addi a2, zero, 17
; CHECK-ASM-NEXT:    mv a3, zero
; CHECK-ASM-NEXT:    cjalr ca4
; CHECK-ASM-NEXT:    clc cra, 8(csp) # 8-byte Folded Reload
; CHECK-ASM-NEXT:    cincoffset csp, csp, 16
; CHECK-ASM-NEXT:    cret
...
; CHECK-ASM-NEXT:    # Label of block must be emitted
; CHECK-ASM-NEXT:    auipcc ca4, %captab_pcrel_hi(stpncpy)
; CHECK-ASM-NEXT:    clc ca4, %pcrel_lo(.LBB4_2)(ca4)
; CHECK-ASM-NEXT:    addi a2, zero, 17
; CHECK-ASM-NEXT:    mv a3, zero
; CHECK-ASM-NEXT:    cjalr ca4
; CHECK-ASM-NEXT:    clc cra, 8(csp) # 8-byte Folded Reload
