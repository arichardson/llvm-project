...
; RUN: opt < %s -instcombine -S | llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+cap-mode,+f,+d - -o - | FileCheck %s --check-prefix=CHECK-ASM
...
; CHECK-IR-NEXT:    call void @llvm.memcpy.p200i8.p200i8.i64(i8 addrspace(200)* noundef nonnull align 1 dereferenceable(17) [[ENDPTR]], i8 addrspace(200)* noundef nonnull align 4 dereferenceable(17) getelementptr inbounds ([17 x i8], [17 x i8] addrspace(200)* @str, i64 0, i64 0), i64 17, i1 false) #[[ATTR3:[0-9]+]]
; CHECK-IR-NEXT:    ret void
;
entry:
...
; CHECK-ASM-NEXT:    # Label of block must be emitted
; CHECK-ASM-NEXT:    auipcc ca3, %captab_pcrel_hi(memcpy)
; CHECK-ASM-NEXT:    clc ca3, %pcrel_lo(.LBB3_2)(ca3)
; CHECK-ASM-NEXT:    addi a2, zero, 17
; CHECK-ASM-NEXT:    cjalr ca3
; CHECK-ASM-NEXT:    clc cra, 0(csp) # 16-byte Folded Reload
; CHECK-ASM-NEXT:    cincoffset csp, csp, 16
...
; CHECK-ASM-NEXT:    # Label of block must be emitted
; CHECK-ASM-NEXT:    auipcc ca3, %captab_pcrel_hi(stpncpy)
; CHECK-ASM-NEXT:    clc ca3, %pcrel_lo(.LBB4_2)(ca3)
; CHECK-ASM-NEXT:    addi a2, zero, 17
; CHECK-ASM-NEXT:    cjalr ca3
; CHECK-ASM-NEXT:    clc cra, 0(csp) # 16-byte Folded Reload
; CHECK-ASM-NEXT:    cincoffset csp, csp, 16
