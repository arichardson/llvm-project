...
; UPDATE_OPT_ARGS: --function-signature
; RUN: opt < %s -instcombine -S | FileCheck %s --check-prefix=CHECK-IR
; RUN: opt < %s -instcombine -S | llc @PURECAP_HARDFLOAT_ARGS@ - -o - | FileCheck %s --check-prefix=CHECK-ASM
...
; CHECK-IR: attributes #[[ATTR3]] = { no_preserve_cheri_tags }
